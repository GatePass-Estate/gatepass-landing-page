  ...
  try {
    ...
    if (!name || typeof name !== "string" || name.trim().length === 0) {
      return Response.json(
        { success: false, error: "Name is required." } satisfies ContactErrorResponse,
        { status: 400 }
      );
    }
    ...
    if (!email || typeof email !== "string" || email.trim().length === 0) {
      return Response.json(
        { success: false, error: "Email is required." } satisfies ContactErrorResponse,
        { status: 400 }
      );
    }
    ...
    if (!message || typeof message !== "string" || message.trim().length === 0) {
      return Response.json(
        { success: false, error: "Message is required." } satisfies ContactErrorResponse,
        { status: 400 }
      );
    }
    ...
    if (!EMAIL_REGEX.test(trimmedEmail)) {
      return Response.json(
        { success: false, error: "Please provide a valid email address." } satisfies ContactErrorResponse,
        { status: 400 }
      );
    }
    ...
    if (!smtpHost || !smtpPort || !smtpUser || !smtpPass || !contactRecipient) {
      ...
      return Response.json(
        { success: false, error: "Server configuration error. Please try again later." } satisfies ContactErrorResponse,
        { status: 500 }
      );
    }
    ...
    return Response.json(
      { success: false, error: "Failed to send message. Please try again later." } satisfies ContactErrorResponse,
      { status: 500 }
    );
  }
}
